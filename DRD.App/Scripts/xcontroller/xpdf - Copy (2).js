myApp.controller("xpdfController", function ($scope, $location, $http, $filter) {
    ...
    $scope.annoSelectorCreateEvent = function () {
        ...
        // pen
        $(".circle-selection").bind("touchstart", selectorMouseDown);
        $(document).bind("touchmove", selectorMouseMove);
        $(document).bind("touchend", selectorMouseUp);
        $(document).bind("touchstart", documentMouseDown);


        $('.rect-selection').bind("dblclick", selectionDblClick);

        // keyboard
        $(document).bind("keydown", delKeyDown);
    ...
    $scope.annoSelectorRemoveEvent = function () {
        ...
        // pen
        $(".circle-selection").unbind("touchstart", selectorMouseDown);
        $(document).unbind("touchmove", selectorMouseMove);
        $(document).unbind("touchend", selectorMouseUp);
        $(document).unbind("touchstart", documentMouseDown);

        $('.rect-selection').unbind("dblclick", selectionDblClick);
        // keyboard
        $(document).unbind("keydown", delKeyDown);
    }
    ...
    function recoverClientValues(e) {
        if (isTouchEvent(e)) {
            ...
            if (e.targetTouches[0].clientX != undefined) {
