    ...
    public class DocumentService : IDocumentService
    {
        ...
        public int Save(DtoDocument prod)
        {
            ...
            using (var db = new DrdContext(_connString))
            {
                ...
                //// prepare data 
                ////
                //var cxold = db.DocumentMembers.Count(c => c.DocumentId == product.Id);
                //var cxnew = prod.DocumentMembers.Count();
                //if (cxold < cxnew)
                //{
                //    var ep = prod.DocumentMembers.ElementAt(0); // get 1 data for sample
                //    for (var x = cxold; x < cxnew; x++)
                //    {
                //        DocumentMember dm = new DocumentMember();
                //        dm.DocumentId = product.Id;
                //        dm.MemberId = ep.MemberId;
                //        dm.FlagPermission = ep.FlagPermission;
                //        dm.FlagAction = ep.FlagAction;
                //        db.DocumentMembers.Add(dm);
                //    }
                //    db.SaveChanges();
                //}
                //else if (cxold > cxnew)
                //{
                //    var dremove = db.DocumentMembers.Where(c => c.DocumentId == product.Id).Take(cxold - cxnew).ToList();
                //    db.DocumentMembers.RemoveRange(dremove);
                //    db.SaveChanges();
                //}
                ////
                //// save data (update)
                ////
                //var dnew = db.DocumentMembers.Where(c => c.DocumentId == product.Id).ToList();
                //int v = 0;
                //foreach (DocumentMember da in dnew)
                //{
                //    var epos = prod.DocumentMembers.ElementAt(v);
                //    da.DocumentId = product.Id;
                //    da.MemberId = epos.MemberId;
                //    da.FlagPermission = epos.FlagPermission;
                //    da.FlagAction = epos.FlagAction;
                //    v++;
                //}
                //db.SaveChanges();
        ...
        //public int Move(long memberId, string docIds, long folderId)
        //{
        //    long[] ids = docIds.Split(',').Select(Int64.Parse).ToArray();
        //    using (var db = new DrdContext(_connString))
        //    {
        //        var docs = db.Documents.Where(c => ids.Contains(c.Id) && c.CreatorId == memberId).ToList();
